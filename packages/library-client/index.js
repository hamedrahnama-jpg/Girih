...
function libraryError(error, fallback) {
  ...
  const message = missing
    ...
    : rls
      ? 'Supabase blocked the shared library save with row-level security. Run the latest shared library migration in the same Supabase project used by Girih Studio.'
...
export async function saveLibraryAsset(supabase, {
  ...
  if (assetId) {
    ...
    if (error && isMissingRpc(error)) {
      throw libraryError(error, 'The shared library version RPC is not installed.');
    }
  ...
  if (error && isMissingRpc(error)) {
    throw libraryError(error, 'The shared library asset RPC is not installed.');
  }
